...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 409, description = "メールアドレスまたはユーザー名が既に存在する"),
...
pub async fn register(
    ...
    .map_err(|e| match e {
        sqlx::Error::Database(ref db_err) if db_err.message().contains("UNIQUE") => {
            AppError::Conflict("username または email が既に使われています".to_string())
        }
        other => AppError::Database(other),
    })?;

    let user = UserResponse { id, username: body.username, email: body.email, role: role.to_string(), created_at: now };
